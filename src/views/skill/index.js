...
const Skills = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios_private.get('/skills', { params })
  ...
  const handleDelete = (id) => {
    toast.warn(
      <div>
        <p>Apakah Anda yakin ingin menghapus keterampilan ini?</p>
  ...
  const deleteData = async (id) => {
    toast.dismiss()
    try {
      await axios_private.delete(`/skills/${id}`)
  ...
  const toggleStatus = async (id) => {
    try {
      await axios_private.patch(`/skills/${id}`)
      ...
      <div className="col-xs-12">
        <div className="card card-data mb-4">
          <div className="card-header">
            <div>
              <h3 className="title">Keterampilan</h3>
              <p className="sub-title">Daftar Keterampilan Anda</p>
            </div>
